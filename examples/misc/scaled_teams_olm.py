...
def shaun_vs_normie(**kwargs):
    ...
    normie = Player(
        ...
        prayers_coll=PrayerCollection(Prayer.augury()),
    )
...
def prayer_routine_comparison(scale: int, trials: int):
    ...
    def switch_to_last(player: Player, last_auto: MonsterStyle, boost_prayer: Prayer):
    ...
    def camp_prot_magic(player: Player, last_auto: MonsterStyle, boost_prayer: Prayer):
    ...
    def camp_prot_ranged(player: Player, last_auto: MonsterStyle, boost_prayer: Prayer):
    ...
    # lads
    lads = (melee_lad, blood_fury_lad, magic_lad) = [
        Player(name=lad) for lad in ("melee lad", "blood fury lad", "magic lad")
    ]
    ...
    for routine_idx, (melee_routine, magic_routine) in enumerate(
        zip(melee_prayer_routines, magic_prayer_routines, strict=True)
    ):
...
if __name__ == "__main__":
    ...
    ax.legend(
        [
            "melee (switch)",
            "melee (camp)",
            "melee [blood fury] (switch)",
            "melee [blood fury] (camp)",
            "mage (switch)",
            "mage (camp)",
        ]
    )
