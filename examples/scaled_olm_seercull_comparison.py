...
from osrs_tools.character import Dummy, Guardian, Player, Olm, OlmHead, OlmMageHand, OlmMeleeHand
...
def seercull_comparison(olms: OlmMageHand | tuple[OlmMageHand], **kwargs):
    ...
    mean_hit = math.floor(ranger.damage_distribution(Dummy(), special_attack=True).mean_hit)
...
def main():
    ...
    seercull_comparison(olms, floatfmt='.2f', data_mode=DataMode.DPT) # , transpose=True, col_labels=col_labels_basis, row_labels=row_labels, floatfmt='.2f', meta_header=meta_header)
