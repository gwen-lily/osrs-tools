...
class StatTest(unittest.TestCase):
	def test_stats(self):
		...
		combat_a = Stats.Combat(10, 20, 30, 40, 50, 60)
		combat_b = Stats.Combat(5, 10, 15, 20, 25, 30)
		combat_c = Stats.Combat(15, 30, 45, 60, 75, 90)
		...
		combat_micro = PlayerStats.Combat.from_highscores(test_rsn)
		combat_maxed = PlayerStats.Combat.maxed_combat()
		self.assertEqual(combat_micro, combat_maxed)

		piety = PlayerStats.StatPrayer.piety()
		self.assertEqual(piety.attack, 1.2)
		self.assertEqual(piety.strength, 1.23)

		rigour = PlayerStats.StatPrayer.rigour()
		self.assertEqual(rigour.ranged_attack, 1.2)
		self.assertEqual(rigour.ranged_strength, 1.23)

		augury = PlayerStats.StatPrayer.augury()
		...
		cerb_stats = MonsterStats.from_bitterkoekje('cerberus')
		cerb_combat_manual = MonsterStats.Combat(600, 220, 220, 100, 220, 220)
		...
		cerb_cc = MonsterStats.CurrentCombat.from_combat(cerb_stats.combat)
		self.assertEqual(cerb_cc.defence, 100)

		myst_stats = MonsterStats.from_de0('skeletal mystic')
		print(myst_stats)
		myst_combat_manual = MonsterStats.Combat(160, 140, 140, 187, 140, 1)
		self.assertEqual(myst_stats.combat, myst_combat_manual)

		olm_mel_stats = MonsterStats.from_de0('great olm (left/melee claw)')
		olm_mel_combat_manual = MonsterStats.Combat(600, 250, 250, 175, 175, 250)
...
class MagicTest(unittest.TestCase):
	def test_sang(self):
		...
		# tolerance self.assertEqual(mage.equipment.aggressive_bonus().magic_strength, 0.23)
