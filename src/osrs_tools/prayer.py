...
class Prayer:
	...
	@classmethod
	def from_osrsbox(cls, *, name: str = None, prayer_id: int = None, drain_effect: int, **kwargs):
		...
		for pp in PRAYERS.all_prayers:
			if pp.name == name or pp.id == prayer_id:
				...
				active_options = {k: LevelModifier(1 + 0.01*v, comment)
...
Augury = Prayer.from_osrsbox(prayer_id=29, defence=25, magic_defence=25, drain_effect=24)
