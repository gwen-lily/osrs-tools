...
class GodSpell(StandardSpell):
    ...
    base_max_hit: DamageValue = field(default_factory=lambda: DamageValue(20), init=False)
    charge_bonus: DamageValue = field(default_factory=lambda: DamageValue(10), init=False)
...
class PoweredSpell(Spell):
    ...
    def max_hit(self, __vis_mag_lvl: Level, /, **kwargs) -> DamageValue:
...
@dataclass(frozen=True)
class TumekenPoweredSpell(PoweredSpell):
    """A spell cast from a shadow of tumeken"""

    name: str = field(default="tumeken's shadow", init=False)
    base_max_hit: DamageValue = field(default_factory=DamageValue.one, init=False)
    attack_speed: int = field(default=5, init=False)

    def max_hit(self, __vis_mag_lvl: Level, /, **kwargs) -> DamageValue:
        max_hit = DamageValue(1) + DamageValue(__vis_mag_lvl // 3)
        return max_hit
