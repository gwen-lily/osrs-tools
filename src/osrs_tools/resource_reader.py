...
def _load_bitterkoekje_bedevere():
    ...
    gear_df = gear_df.rename(
        columns={gear_df.columns[0]: item_name_header}
    ...
    for ic in integer_columns:
        ...
        try:
            ...
            gear_df[
                ic
            ] = 0  # magic dart is listed as "mdart", only value I know to break this
...
def _load_bitterkoekje_npc():
    ...
    int_columns_range = np.append(
        np.append(np.arange(3, 9 + 1), np.arange(11, 25 + 1)), np.arange(29, 29 + 1)
    )
...
def lookup_normal_monster_by_name(
    name: str, npc_df: pd.DataFrame = None
) -> pd.DataFrame:
